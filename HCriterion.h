...
public:
	...
	Criterion(double epsilon = 0.001) :eps(epsilon) {};
	virtual const bool stop(vector<double>, vector<double>, Function&, int, int = 0) const = 0;
};

class Criterion_grad_norm :public Criterion {
public:
	Criterion_grad_norm(double epsilon = 0.001) :Criterion(epsilon) {};
	virtual const bool stop(vector<double>, vector<double>, Function&, int, int = 0)  const override;
};
...
public:
	...
	virtual const bool stop(vector<double> x, vector<double> y, Function& f, int num_iteration, int num_iteration_without_change = 0) const override;
};
