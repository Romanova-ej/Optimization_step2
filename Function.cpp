...
double Function::getDirectionalDerivative(const vector<double>& x, const vector<double>& p) const {
...
vector<double> Fun2::getGradient(const vector<double>& x) {//тут пусть дана явно
	return 2 * x;
}
...
inline const double Fun3::f(const vector<double>& x) const {//прост квадратичная  решения: x1=2x2,  f=0;
...
const char* Fun4::getFunctionName() const { return "(x2-x1^2)^2+100*(1-x1)^2"; };
...
vector<double> rightD(r, r + 4);
D.setArea(leftD, rightD);
}// //функция №7 x=(0,0,0,0), f=0;
const char* Fun5::getFunctionName() const { return "(x1+10x2)^2+5(x3-x4)^2+(x2-2x3)^4+10(x1-x4)^4"; };
